 ...
 */
public class ToString<C, O, P, EL, PM, S, COA, SSA, CT>
	...
	private static String[] reservedNameList = { "and", "else", "endif",
		"false", "if", "implies", "in", "invalid", "let", "not", "null",
		"or", "self", "then", "true", "xor", "Bag", "Boolean",
		"Collection", "Integer", "OclAny", "OclInvalid", "OclVoid",
		"OrderedSet", "Real", "Sequence", "Set", "String", "Tuple",
		"UnlimitedNatural", "body", "package", "context", "init" };

private static String[] specialOps = { "=", "<=", "<", ">=", ">", "or",
		"and", "implies", "xor", "<>" };
	/**
	 ...
	 */
	protected ToString(Environment<?, C, O, P, EL, PM, S, COA, SSA, CT, ?, ?> env) {
	    this.env = env;
	...
	ToString<C, O, P, EL, PM, S, COA, SSA, CT> getInstance(
			Environment<?, C, O, P, EL, PM, S, COA, SSA, CT, ?, ?> env) {
		
		return new ToString<C, O, P, EL, PM, S, COA, SSA, CT>(env);
	}
	...
	ToString<C, O, P, EL, PM, S, COA, SSA, CT> getInstance(
		...
		return new ToString<C, O, P, EL, PM, S, COA, SSA, CT>(
	...
	protected String getName(Object named) {
		...
		if (named==null) return null; else
		{
		String ll ="";
		
	try{
		
		ll=((ENamedElementImpl)named).eProxyURI().toString();
	}catch(Exception e){
		
	ll=((ENamedElementImpl)named).getName();	
	return ll;
	}
	    
	  return (uml == null | ll==null)? ll.substring(ll.lastIndexOf('/')+1,ll.length()): uml.getName(named);
		//return (uml == null)? null: uml.getName(named);
		}	
	}
    ...
    protected String getQualifiedName(Object named) {
        ...
        return (uml == null)? getName(named) : uml.getName(named);
    }
  ...
  @Override  
    protected String handleOperationCallExp(OperationCallExp<C,O> oc,
		...
		EClassifier sourceType = (EClassifier) (source != null ? source.getType() : null);
		EOperation oper = (EOperation) oc.getReferredOperation();
		String opN=null;
		if (oper!=null)
		try { opN=((EOperationImpl)oper).eProxyURI().toString();}catch(Exception e){
			opN=((ENamedElementImpl)oper).getName();
		...
		boolean specialOp = isSpecialOp(opN);

	String opName = getOpName(oper);
	if (specialOp) {
		result.append(sourceResult);
		result.append(" "+opN+" ");
		
	} else if (opName.equals("_not")) {

		result.append("not " + sourceResult);

	} else {
		result.append(sourceResult);
		result.append(sourceType instanceof CollectionType<?, ?> ? "->" : "."); //$NON-NLS-1$ //$NON-NLS-2$
		result.append(opN==null? null: opN.substring(opN.lastIndexOf('/')+1,opN.length()));
		
	}

	if (!specialOp && !opName.equals("_not"))// &&
												// !opName.endsWith("()"))
		result.append('(');

	if (specialOp)
		result.append(' ');
	for (Iterator<String> iter = argumentResults.iterator(); iter
			.hasNext();) {
...
return maybeAtPre((FeatureCallExp<C>) oc, result.toString());
}
    ...
    public String visitEnumLiteralExp(EnumLiteralExp<C, EL> el) {
		...
		if (getName(((EClassifier) el.getType()).getEPackage()).equals("oclstdlib")) 
			return getQualifiedName(l);
		else 
	     return 	getName(((EClassifier) el.getType()).getEPackage().getESuperPackage())+"::"+getName(((EClassifier) el.getType()).getEPackage())+
		"::"+getName(el.getType())+"::"+ getQualifiedName(l);
    ...
    public String visitVariableExp(VariableExp<C, PM> v) {
		...
		String result = (vd == null) ? v.getName() : vd.getName();
    ...
    protected String handleVariable(Variable<C,PM> vd, String initResult) {
		...
		if (type != null) {
			result += " : " +getName(((EClassifier) type).getEPackage().getESuperPackage())+"::"+getName(((EClassifier) type).getEPackage())+
					"::"+ getName(type);//$NON-NLS-1$
    ...
    public String visitTypeExp(TypeExp<C> t) {
		...
		if (getName(((EClassifier) t.getReferredType()).getEPackage()).equals("oclstdlib")) 
			return getQualifiedName(t.getReferredType());
		else 
	     return 	getName(((EClassifier) t.getReferredType()).getEPackage().getESuperPackage())+"::"+getName(((EClassifier) t.getReferredType()).getEPackage())+
		"::"+ getName(t.getReferredType());
...
*/
	...
	protected List<EObject> getConstrainedElements(CT constraint) {
		return (List<EObject>) ((uml == null)? null : uml.getConstrainedElements(constraint));
	}
	...
	///////////////////
	private String getOpName(EOperation oper) {
		String name = getName(oper);
		return isKeyword(name) && !isSpecialOp(name) ? "_" + name : name;
	}

	public boolean isSpecialOp(String opN) {
		for (String n : specialOps) {
			if (n.equals(opN)) {
				return true;
			}
		}
	...
	private boolean isKeyword(String propertyName) {
		for (String reserved : reservedNameList) {
			if (reserved.equals(propertyName))
				return true;
		}
		return false;
	}
